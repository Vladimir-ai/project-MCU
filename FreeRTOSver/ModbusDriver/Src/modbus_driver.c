...
{
  uint16_t crc_expected_value = CONCAT_TWO_BYTES(buf_start[buf_len - 3], buf_start[buf_len - 2]);
...
static modbus_status_t modbus_parse_read_function_and_call_cb(const uint8_t const *buf,
                                                              const buf_len,
                                                              const uint8_t const *rsp_buf,
                                                              const uint8_t rsp_buf_size,
                                                              const modbus_read_call_t read_callback)
{
  ...
  if (buf_len == 4)
  {
    ...
    outputs_num = CONCAT_TWO_BYTES(buf[3], buf[4]);
                                                    ...
                                                    const uint8_t payload_len)
{
  ...
  if (ret == MODBUS_STATUS_OK)
  {
    ...
    rsp_buf[rsp_buf_size + 2] = crc >> 8 & 0xFFU;
    rsp_buf[rsp_buf_size + 3] = crc & 0xFFU;

    g_modbus_ctx.tx_cb(rsp_buf, rsp_buf_size + 4);
  }
...
modbus_status_t modbus_serial_rx(const uint8_t const *buf_start, const uint8_t buf_len)
{
  ...
  ret = modbus_validate_crc(buf, buf_len);
  payload_len -=2;
